...
1. Create the plugin repository. For managed hosted automation, protect its configured `DEFAULT_BRANCH` (`main` by default).
2. Import a complete, verified published foundation tree into `.wp-plugin-base/` using the [manual import procedure](manual-foundation-import.md). Autogenerated host archives omit tracked templates and are not complete imports.
