...
Readiness validation also reports and enforces admin UI asset budgets for the built `assets/admin-ui/` tree. Override `WP_PLUGIN_BASE_ADMIN_UI_MAX_SCRIPT_BYTES`, `WP_PLUGIN_BASE_ADMIN_UI_MAX_STYLE_BYTES`, or `WP_PLUGIN_BASE_ADMIN_UI_MAX_TOTAL_BYTES` only when the larger runtime payload is intentional and reviewed.
