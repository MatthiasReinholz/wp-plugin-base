...
wp_plugin_base_is_supported_config_key() {
  case "$1" in
FOUNDATION_REPOSITORY|FOUNDATION_VERSION|FOUNDATION_RELEASE_SOURCE_PROVIDER|FOUNDATION_RELEASE_SOURCE_REFERENCE|FOUNDATION_RELEASE_SOURCE_API_BASE|FOUNDATION_RELEASE_SOURCE_SIGSTORE_ISSUER|AUTOMATION_PROFILE|DEFAULT_BRANCH|AUTOMATION_PROVIDER|AUTOMATION_API_BASE|TRUSTED_GIT_HOSTS|PLUGIN_RUNTIME_UPDATE_PROVIDER|PLUGIN_RUNTIME_UPDATE_SOURCE_URL|PLUGIN_NAME|PLUGIN_SLUG|MAIN_PLUGIN_FILE|README_FILE|ZIP_FILE|PHP_VERSION|NODE_VERSION|PRODUCTION_ENVIRONMENT|PHP_RUNTIME_MATRIX|PHP_RUNTIME_MATRIX_MODE|PHPSTAN_MEMORY_LIMIT|VERSION_CONSTANT_NAME|POT_FILE|POT_PROJECT_NAME|WORDPRESS_ORG_SLUG|WORDPRESS_READINESS_ENABLED|WORDPRESS_QUALITY_PACK_ENABLED|WORDPRESS_SECURITY_PACK_ENABLED|GITHUB_CODE_SCANNING_UPLOAD_ENABLED|RELEASE_READINESS_MODE|WOOCOMMERCE_QIT_ENABLED|WOOCOMMERCE_COM_PRODUCT_ID|WOOCOMMERCE_COM_ENDPOINT_TIMEOUT_SECONDS|GITHUB_RELEASE_UPDATER_ENABLED|GITHUB_RELEASE_UPDATER_REPO_URL|RUNTIME_CLASS_PREFIX|REST_OPERATIONS_PACK_ENABLED|REST_API_NAMESPACE|REST_ABILITIES_ENABLED|ADMIN_UI_PACK_ENABLED|ADMIN_UI_STARTER|ADMIN_UI_EXPERIMENTAL_DATAVIEWS|ADMIN_UI_NPM_AUDIT_LEVEL|WP_PLUGIN_BASE_PLUGIN_CHECK_CHECKS|WP_PLUGIN_BASE_PLUGIN_CHECK_EXCLUDE_CHECKS|WP_PLUGIN_BASE_PLUGIN_CHECK_CATEGORIES|WP_PLUGIN_BASE_PLUGIN_CHECK_IGNORE_CODES|WP_PLUGIN_BASE_PLUGIN_CHECK_STRICT_WARNINGS|WP_PLUGIN_BASE_PLUGIN_CHECK_SEVERITY|WP_PLUGIN_BASE_PLUGIN_CHECK_ERROR_SEVERITY|WP_PLUGIN_BASE_PLUGIN_CHECK_WARNING_SEVERITY|EXTRA_ALLOWED_HOSTS|WP_PLUGIN_BASE_SECURITY_SUPPRESSIONS_FILE|PACKAGE_INCLUDE|PACKAGE_EXCLUDE|CHANGELOG_HEADING|CODEOWNERS_REVIEWERS|DISTIGNORE_FILE|BUILD_SCRIPT|BUILD_SCRIPT_ARGS|BUILD_OUTPUTS|BUILD_OUTPUT_MANIFEST|PHPDOC_VERSION_REPLACEMENT_ENABLED|PHPDOC_VERSION_PLACEHOLDER|CHANGELOG_MD_SYNC_ENABLED|CHANGELOG_SOURCE|SIMULATE_RELEASE_WORKFLOW_ENABLED|GLOTPRESS_TRIGGER_ENABLED|GLOTPRESS_URL|GLOTPRESS_PROJECT_SLUG|GLOTPRESS_FAIL_ON_ERROR|DEPLOY_NOTIFICATION_ENABLED|DEPENDABOT_ECOSYSTEMS)
      return 0
      ;;
    *)
      return 1
...
wp_plugin_base_load_config() {
  ...
  AUTOMATION_PROFILE="${AUTOMATION_PROFILE:-managed}"
  ...
  GITHUB_CODE_SCANNING_UPLOAD_ENABLED="${GITHUB_CODE_SCANNING_UPLOAD_ENABLED:-true}"
  RELEASE_READINESS_MODE="${RELEASE_READINESS_MODE:-standard}"
  # The optional manual QIT workflow is supported only by the GitHub host.
  ...
  DEPENDABOT_ECOSYSTEMS="${DEPENDABOT_ECOSYSTEMS:-auto}"
  RUNTIME_CLASS_PREFIX="${RUNTIME_CLASS_PREFIX:-}"
  ...
  BUILD_OUTPUT_MANIFEST="${BUILD_OUTPUT_MANIFEST:-}"
...
# All externally mutating publication/update entrypoints must call this after loading
# config. Credentials never grant capabilities disabled by the managed profile.
wp_plugin_base_require_managed_automation() {
  if [ "${AUTOMATION_PROFILE:-managed}" != managed ]; then
    echo "${1:-Hosted automation} is disabled by AUTOMATION_PROFILE=local." >&2
    return 1
  fi
}
...
wp_plugin_base_canonicalize_path() {
  ...
  # Walk every component, including nonexistent tails and the final component.
  # Resolving a parent first and appending its missing suffix leaves traversal
  # segments unchecked and overlooks file symlinks pointing outside the root.
  while [ -n "$remaining" ]; do
    ...
    if [ -L "$candidate" ]; then
      ...
      if [ "$link_count" -gt 40 ]; then
        echo "Unable to resolve path with excessive symbolic links: $path" >&2
        return 1
      fi
      if ! link_target="$(readlink "$candidate")"; then
        echo "Unable to resolve symbolic link: $candidate" >&2
        return 1
      fi
      case "$link_target" in
        /*) resolved="" ;;
      esac
      remaining="$link_target${remaining:+/$remaining}"
    else
      resolved="$candidate"
    fi
  done
...
wp_plugin_base_assert_path_within_root() {
  ...
  if ! canonical_path="$(wp_plugin_base_canonicalize_path "$path")"; then
    return 1
  fi
