...
assert_file_contains_literal "$ROOT_DIR/.github/workflows/prepare-release.yml" 'Release base ref $BASE_REF must be the default branch or a protected branch.' "Reusable prepare-release workflow must reject untrusted base refs before checkout."
...
assert_file_contains_literal "$ROOT_DIR/templates/child/.github/workflows/prepare-release.yml" 'Release base ref $BASE_REF must be __DEFAULT_BRANCH__ or a protected branch.' "Managed prepare-release workflow must reject untrusted base refs before checkout."
